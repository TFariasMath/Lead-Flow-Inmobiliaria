...
export default function HistoryDock() {
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        "absolute bottom-0 right-0 w-20 h-20 bg-blue-500/5 rounded-full blur-2xl transition-all duration-500 group-hover:scale-150 group-hover:bg-blue-500/10 cursor-pointer",
        isHovered && "opacity-0"
      )} />
      
      {/* Visual Indicator (Small floating pill) */}
      {!isHovered && (
        <div className="absolute bottom-2 right-2 w-3 h-3 rounded-full bg-blue-500/20 border border-blue-500/40 animate-pulse flex items-center justify-center">
          <Clock className="w-1.5 h-1.5 text-blue-400" />
        </div>
      )}
        ...
        <div className="bg-slate-900/90 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center gap-2 ring-1 ring-white/5">
          ...
          <div className="flex items-center gap-1.5 p-1">
            {history.slice(0, 3).map((lead) => (
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-600/20 to-indigo-600/20 border border-blue-500/20 flex items-center justify-center text-[11px] font-black text-blue-400 group-hover/item:scale-110 group-hover/item:shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all duration-500">
                ...
                <div className="text-left">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={cn(
                      ...
                      lead.status === "nuevo" ? "bg-blue-400" : "bg-emerald-400"
                    )} />
