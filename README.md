...
El acceso al sistema está blindado mediante una interfaz de **"login"**, diseñada para proyectar seguridad y control desde el primer contacto.
...
*   **Control de Acceso Basado en Roles (RBAC):** El sistema identifica automáticamente si el usuario es un **Administrador** o un **Agente de Ventas**.
...
### 🖥️ Dashboard: El Poder de la Vista Única

El dashboard principal ha sido diseñado bajo una filosofía de **Control Situacional**.

![Dashboard Overview](./frontend/public/docs/dashboard1.png)

#### 🎴 Navegación Inmersiva y Paneles de Control
El sistema utiliza una arquitectura de paneles deslizables diseñados para mantener el enfoque operativo.

*   **Sidebar (Navegación Primaria):** Una pieza de ingeniería visual diseñada bajo el concepto de **"Macro-Módulos"**. No es una barra estática, sino una interfaz dinámica que se oculta y revela mediante un área de disparo sensible al movimiento.
    *   **Estructura y Estética:** Implementa un diseño moderno con fondo oscuro y desenfoque. Cuenta con una jerarquía vertical clara: Perfil de Usuario en la cima, Macro-Grupos operativos al centro y branding de marca en la base.
    *   **Navegación por Macro-Grupos:**
        ...
        *   **Módulo Marketing (`marketing`):** Orquesta Landing Pages dinámicas, Campañas de Brochures PDF y el Catálogo de Propiedades.
        *   **Módulo Laboratorio (`lab`):** Espacio técnico para analíticas de rendimiento, Email Sandbox y Webhook Logs (Modo Quirófano).
        *   **Módulo Ajustes (`config`):** Control total sobre Usuarios y Roles (RBAC).
    *   **Funcionalidades Inteligentes:** Incluye indicadores visuales para ubicación espacial, flyouts de perfil para gestión de sesión y pre-carga de datos al hacer hover sobre los módulos para una carga instantánea.
*   **Right Sidebar (Panel de Contexto Operativo):** Un panel diseñado para la supervisión en tiempo real. **Nota:** Este panel es exclusivo de la vista de Dashboard Principal para optimizar el foco operativo.
    ...
    *   **Feed de Actividad:** Una línea de tiempo dinámica que registra cada interacción crítica (captura de webhooks, cambios de estado, notas de vendedores). Proporciona feedback visual inmediato sobre el estado de la operación sin necesidad de navegar a módulos profundos.
    *   **Estética de Enfoque:** Implementa una transición de deslizamiento lateral que se integra con el diseño moderno general, manteniendo el contexto del negocio siempre a la vista.
*   **Top Drawer (Métricas de Tráfico):** Invocado desde la parte superior, este panel despliega el análisis técnico del pulso digital. Permite auditar el rendimiento de cada landing page y visualizar picos de tráfico sin necesidad de abandonar la lista de trabajo actual, ideal para supervisiones rápidas.
*   **Dashboard Dock (Historial Rápido):** Situado en la esquina inferior derecha, funciona como una "memoria de acceso rápido". Registra automáticamente los últimos leads visitados, permitiendo saltar de vuelta a una negociación previa con un solo clic, eliminando la necesidad de re-búsquedas constantes.
...
*   **Trazabilidad de Errores:** En caso de fallo, la consola expone el error técnico exacto reportado por el servidor, facilitando el diagnóstico sin necesidad de revisar logs pesados.
...
*   **Aislamiento (Isolation):** El webhook se guarda inmediatamente como un `WebhookLog` crudo. El sistema responde `200 OK` en <50ms para evitar timeouts del proveedor externo.
...
*   **Bloqueo de Base de Datos:** Para evitar que dos webhooks del mismo lead al mismo milisegundo creen duplicados, el sistema usa bloqueos de fila (`FOR UPDATE`) en PostgreSQL. El primer proceso "atrapa" al lead y los demás deben esperar en cola.
...
*   **Ancla Fluida (`contact_email`):** Es el correo que el vendedor edita para la gestión comercial activa. Permite que el sistema reconozca al lead si este re-ingresa usando una identidad que el vendedor ya "mapeó" manualmente.
...
En ambos escenarios, el sistema evita el duplicado y anexa la nueva interacción al historial existente, manteniendo la integridad del viaje del cliente.

#### Motor Round Robin Determinista
...
1.  **Intervención Directa:** Cuando un lead falla por datos mal formados, el administrador puede acceder al registro y acceder al JSON crudo.
...
#### ¿Cómo funciona la recepción (`api/v1/webhooks/receive/`)?
...
Capacidad de generar folletos personalizados al vuelo basándose en el perfil del lead y la campaña:
...
#### Catálogo de Activos Inmobiliarios
...
#### 4. `/tools` (Utilidades de Ingeniería)
